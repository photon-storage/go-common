...
//
// Tests for deep equality using reflected pbc. The map argument tracks
...
func deepValueBaseTypeEqual(v1, v2 reflect.Value) bool {
	...
	case reflect.Uint64:
		switch v1.Type().Name() {
		case "Epoch":
			return v1.Interface().(pbc.Epoch) == v2.Interface().(pbc.Epoch)
		case "Slot":
			return v1.Interface().(pbc.Slot) == v2.Interface().(pbc.Slot)
		case "ValidatorIndex":
			return v1.Interface().(pbc.ValidatorIndex) == v2.Interface().(pbc.ValidatorIndex)
		case "CommitteeIndex":
			return v1.Interface().(pbc.CommitteeIndex) == v2.Interface().(pbc.CommitteeIndex)
		}
